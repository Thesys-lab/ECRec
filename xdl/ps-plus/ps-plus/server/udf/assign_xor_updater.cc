...
class AssignXorUpdater : public SimpleUdf<vector<Slices>, vector<Tensor> > {
 public:
  virtual Status SimpleRun(
    ...
    if (sslices.size() != grad_tensors.size()) {
      return Status::ArgumentError("AssignXorUpdater: slices and other size not match");
    }
...
SIMPLE_UDF_REGISTER(AssignXorUpdater, AssignXorUpdater);
