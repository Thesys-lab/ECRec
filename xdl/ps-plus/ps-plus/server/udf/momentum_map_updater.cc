...
using std::vector;
std::unordered_map<float *, float> data_map;
std::unordered_map<float *, float> acc_map;
class MomentumMapUpdater : public SimpleUdf<vector<Slices>, vector<Tensor>, vector<double>, vector<double>, vector<bool> > {
 public:
    ...
    for (size_t si = 0; si < sslices.size(); si++) {
            ...
            CASES(data_tensor->Type(), MultiThreadDo(slices.slice_id.size(), [&](const Range& r) {
                      for (size_t i = r.begin; i < r.end; i++) {
                        ...
                        float* data = data_tensor->Raw<float>(slice);
                        float* acc = acc_tensor->Raw<float>(slice);
                        ...
                        if (use_nesterov) {
                          for (size_t j = 0; j < slices.slice_size; j++) {
                            ...
                            acc_map[acc] = acc_map[acc] * momentum + *grad;
                            data_map[data] -= *grad * learning_rate + acc_map[acc] * momentum * learning_rate;
                            data++; acc++; grad++;
                          }
                        } else {
                          for (size_t j = 0; j < slices.slice_size; j++) {
                            ...
                            acc_map[acc] = acc_map[acc] * momentum + *grad;
                            data_map[data] -= acc_map[acc] * learning_rate;
                            data++; acc++; grad++;
                          }
                        }
                      }
...
SIMPLE_UDF_REGISTER(MomentumMapUpdater, MomentumMapUpdater);
