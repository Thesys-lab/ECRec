      ...
      for (size_t si = range.begin; si < range.end; ++si) {
        ...
        for (size_t i = 0; i < to.parts.size(); i++) {
          ...
          if (part.server == id) {
            ...
            if (!st.IsOk()) {
              if ((st.Code() == Status::ErrorCode::kUnknown || st.Code() == Status::ErrorCode::kNotFound) &&
                  vi.args.find("save") != vi.args.end() && vi.args["save"] == "false") {
...
Status CheckpointUtils::SaveVariables(
  ...
  for (auto &&item : vars) {
    ...
    ThreadPool::Global()->Schedule([&, name] {
        ...
        if (part == -1) {
          CK_CHECK_STATUS(status, Status::ArgumentError(
                  "Not found variable[" + name + "] part[" + std::to_string(id) +
...
Status CheckpointUtils::MergeLoadVariable(const std::string &name, const VariableInfo &info, size_t beg, size_t end,
                                          std::unique_ptr<Variable> *result_variable, size_t server_id) {
  ...
  LOG(INFO) << info.name << ", LoadVariable takes "
  ...
  if (info.type == VariableInfo::Type::kIndex) {
    ...
    var.data = Tensor(variables[0]->variable.data.Type(), shape,
                      variables[0]->variable.data.GetInitializer()->Clone());
    ...
    for (const auto &lvs : variables) {
      if (lvs->beg <= beg) {
        QuickMemcpy(var.data.Raw<char>(), lvs->variable.data.Raw<char>() + (beg - lvs->beg) * slice_size,
                    (lvs->clip_end - lvs->clip_beg) * slice_size);
      } else {
        QuickMemcpy(var.data.Raw<char>() + (lvs->beg - beg) * slice_size, lvs->variable.data.Raw<char>(),
                    (lvs->clip_end - lvs->clip_beg) * slice_size);
      }
    }
    ...
    for (auto &iter : variables[0]->variable.slots) {
      if (iter.second.joiner == Variable::SlotJoiner::kAnyOne) {
        var.slots[iter.first] = Variable::Slot{.tensor = std::unique_ptr<Tensor>(
                new Tensor(*iter.second.tensor)), .joiner = iter.second.joiner};
      } else {
        ...
        var.slots[iter.first] = Variable::Slot{.tensor = std::unique_ptr<Tensor>(
                new Tensor(t->Type(), s, t->GetInitializer()->Clone())), .joiner = iter.second.joiner};
        ...
        for (auto &lvs : variables) {
          if (lvs->beg <= beg) {
            QuickMemcpy(var.slots[iter.first].tensor->Raw<char>(),
                        lvs->variable.slots[iter.first].tensor->Raw<char>() + (beg - lvs->beg) * ssize,
                        (lvs->clip_end - lvs->clip_beg) * ssize);
          } else {
            QuickMemcpy(var.slots[iter.first].tensor->Raw<char>() + (lvs->beg - beg) * slice_size,
                        lvs->variable.slots[iter.first].tensor->Raw<char>(),
                        (lvs->clip_end - lvs->clip_beg) * ssize);
          }
        }
      }
    ...
    time_end = std::chrono::system_clock::now();
    LOG(INFO) << info.name << ", load hash variable total, takes "
              << std::chrono::duration_cast<std::chrono::seconds>(time_end - time_start).count();
    return Status::Ok();
  }
}

Status CheckpointUtils::LoadHashVariable(const std::vector<std::unique_ptr<LoadVariableStruct>> &variables,
                                         const std::string &name, const VariableInfo &info, size_t beg, size_t end,
                                         std::unique_ptr<Variable> &result_variable) {
  ...
  LOG(INFO) << name << ", CalMaxSize, takes "
            << std::chrono::duration_cast<std::chrono::seconds>(time_end - time_start).count() << ", max_size is "
            << max_size;
  ...
  Variable *var = new Variable(
          new Tensor(t.Type(), data_shape, t.GetInitializer()->Clone(), Tensor::TType::kSegment, false),
          new WrapperData<std::unique_ptr<HashMap> >(hashmap), name);
  std::unordered_map<std::string, Variable::Slot> slots;
  for (const auto &iter : variables[0]->variable.slots) {
    if (iter.second.joiner == Variable::SlotJoiner::kAnyOne) {
      slots[iter.first] = Variable::Slot{.tensor = std::unique_ptr<Tensor>(
              new Tensor(*iter.second.tensor)), .joiner = iter.second.joiner};
    } else {
      ...
      slots[iter.first] = Variable::Slot{.tensor = std::unique_ptr<Tensor>(
              new Tensor(tt.Type(), tt_shape, tt.GetInitializer()->Clone(), Tensor::TType::kSegment,
                         false)), .joiner = iter.second.joiner};
    }
  }
  time_end = std::chrono::system_clock::now();
  LOG(INFO) << name << ", initialize takes "
            << std::chrono::duration_cast<std::chrono::seconds>(time_end - time_start).count();
  ...
  for (size_t i = 0; i < variables.size(); i++) {
    ...
    size_t slice_size =
            SizeOfType(var->GetData()->Type()) * var->GetData()->Shape().NumElements() / var->GetData()->Shape()[0];
    for (size_t j = 0; j < ids.size(); j++) {
      ...
      for (auto &slot : slots) {
        if (slot.second.joiner == Variable::SlotJoiner::kVariableLike) {
          size_t ssize = SizeOfType(slot.second.tensor->Type()) * slot.second.tensor->Shape().NumElements() /
                         slot.second.tensor->Shape()[0];
...
int64_t CheckpointUtils::CalMaxSize(const std::vector<std::unique_ptr<LoadVariableStruct> > &variables,
                                    const std::string &name, size_t begin, size_t end,
  ...
  LOG(INFO) << name << ", variables.size() " << variables.size() << " begin " << begin << " end " << end
            << " total " << total;
  return total;
}
...
CheckpointUtils::LoadVariableWithRedundancy(std::string name, size_t part, VariableStruct *var, size_t server_id) {
  ...
  // TODO: need to make sure info is the client side info.
...
Status CheckpointUtils::SaveVariable(const std::string &checkpoint, const std::string &var_name, size_t part,
                                     VariableStruct *var) {
...
Status CheckpointUtils::LoadTensor(const std::string &name, FileSystem::ReadStream *s,
                                   VariableStruct::SlicerType slicer_type, Tensor *data) {
  DataType type;
...
Status CheckpointUtils::SaveTensor(FileSystem::WriteStream *s, const Tensor &data) {
  ...
  if (data.TensorType() == Tensor::TType::kContinuous) {
    ...
    for (size_t i = 0; i < tensor_shape[0] / data.SegmentSize(); i++) {
      PS_CHECK_STATUS(
              s->Write(data.Raw<char>(i * data.SegmentSize()), data.SegmentSize() * slice_size * SizeOfType(type)));
    }
  } else {
...
std::unordered_map<std::string, Variable::Slot>
CheckpointUtils::CloneSlots(const std::unordered_map<std::string, Variable::Slot> &slots) {
  std::unordered_map<std::string, Variable::Slot> ret;
  for (auto &&item : slots) {
    ret[item.first] = Variable::Slot{.tensor = std::unique_ptr<Tensor>(
            new Tensor(*item.second.tensor)), .joiner = item.second.joiner};
  }
  return std::move(ret);
}
}
